package com.mh.notification.mock.service;
...
import com.mh.notification.mock.api.MockSendFailResponse;
import com.mh.notification.mock.api.MockSendRequest;
import com.mh.notification.mock.api.MockSendSuccessResponse;
import com.mh.notification.mock.config.MockMode;
import com.mh.notification.mock.config.MockProperties;
...
public class MockSendService {
    ...
    public MockSendSuccessResponse handleSend(MockSendRequest request) {
        ...
        if (fail) {
            throw buildFailureException(request, startedAtMillis);
        }

        long latencyMs = System.currentTimeMillis() - startedAtMillis;
        MockSendSuccessResponse response = new MockSendSuccessResponse(
                "SUCCESS",
                request.requestId(),
    ...
    private boolean shouldDelay() {
        return properties.getLatency().isEnabled() && randomHit(properties.getLatency().getProbability());
    }

    private boolean shouldFail() {
        return properties.getFailure().isEnabled() && randomHit(properties.getFailure().getProbability());
    }
    ...
    private void logRequest(MockSendRequest request, String behavior, int status, long latencyMs) {
        if (properties.getLog().isIncludeMaskedMessagePreview()) {
            log.info(
                    ...
                    safeText(request.channelType().name()),
                    safeText(request.receiver()),
                    request.message().length(),
                    maskedPreview(request.message()),
                    behavior,
                    status,
                    latencyMs
            );
    ...
    private String maskedPreview(String message) {
        ...
        int length = Math.min(message.length(), maxLength);
